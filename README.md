...
[![Version](https://img.shields.io/badge/version-0.2.1-blue.svg)](https://github.com/hrodrig/kzero/releases)
...
| Format | Example (tag **`v0.2.1`** in the URL path; artifact basename includes the same **`v0.2.1`**) |
|--------|------------------------------------------------------------------|
| **`.deb`** | `curl -fsSL -o /tmp/kzero_v0.2.1_linux_amd64.deb https://github.com/hrodrig/kzero/releases/download/v0.2.1/kzero_v0.2.1_linux_amd64.deb` then `sudo apt install /tmp/kzero_v0.2.1_linux_amd64.deb` |
| **`.rpm`** | `curl -fsSLO https://github.com/hrodrig/kzero/releases/download/v0.2.1/kzero_v0.2.1_linux_amd64.rpm` then `sudo rpm -Uvh kzero_v0.2.1_linux_amd64.rpm` or `sudo dnf install ./kzero_v0.2.1_linux_amd64.rpm` |
| **`.tar.gz` (Linux)** | `curl -fsSLO https://github.com/hrodrig/kzero/releases/download/v0.2.1/kzero_v0.2.1_linux_amd64.tar.gz` then `tar xzf kzero_v0.2.1_linux_amd64.tar.gz` and run **`./kzero`** from the extracted tree (see **`share/examples/kzero/kzero.sample.yml`**) |
| **`.tar.gz` (macOS)** | `curl -fsSLO https://github.com/hrodrig/kzero/releases/download/v0.2.1/kzero_v0.2.1_darwin_amd64.tar.gz` (or **`…_darwin_arm64.tar.gz`** on Apple silicon) |
...
**Windows:** use the **`.zip`** asset for your arch (for example **`kzero_v0.2.1_windows_amd64.zip`**), unpack, run **`kzero.exe`** where **`kubectl`** is available.

**Docker:** `docker pull ghcr.io/hrodrig/kzero:v0.2.1` (match the image tag to the **[release](https://github.com/hrodrig/kzero/releases)** you want). Published images use **`gcr.io/distroless/static-debian12:nonroot`** (static **`kzero`** binary only: no shell, no BusyBox/Alpine runtime). **`Dockerfile`** in this repo uses the same final stage. Package: [ghcr.io/hrodrig/kzero](https://github.com/hrodrig/kzero/pkgs/container/kzero).
...
Use a **release tag** instead of `@latest` if you want a pinned version (for example `@v0.2.1`). Module reference: [pkg.go.dev/github.com/hrodrig/kzero](https://pkg.go.dev/github.com/hrodrig/kzero).
...
3. On **`main`**: create an annotated tag (e.g. `git tag -a v0.2.1 -m "Release 0.2.1"`) and **`git push origin v0.2.1`**. The **Release** workflow runs **`make release-check`** then **GoReleaser** (binaries + **`ghcr.io/hrodrig/kzero`**).
